  ...
  return (
    <div className="flex flex-col relative min-h-screen">
            ...
            >
              ...
              <div className="w-full flex items-center mt-4">
                ...
                <div className="h-10 w-10 border-2 p-1 border-gray-500 rounded-full">
                  <img src={contestant.partyLogo} alt={contestant.name} className="w-full h-full"/>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="flex  absolute bottom-0 left-0 right-0 bg-[#D9D9D999]  p-6">
        ...
        <button disabled={!selected} onClick={() => handleConfirmVote()} className={` ${!selected ? "cursor-not-allowed bg-[#39FF14B2]/40 " : "cursor-pointer bg-[#39FF14B2] hover:bg-[#39FF14B2]/50"}  flex items-center gap-2  font-700 font-bold text-gray-900 text-sm px-4 py-2 rounded-lg"`}>
          Confirm Your Vote
      ...
      {/* confirm modal */}
      {confirmModal && (<ConfirmVote selected={selected} handleConfirmVote={handleConfirmVote} />)}
    </div>
  );
};
